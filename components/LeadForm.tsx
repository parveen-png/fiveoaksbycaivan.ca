...
export function LeadForm({ idPrefix }: LeadFormProps) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.captured && data.submissionId && !data.alreadyRecorded) {
        trackGenerateLead(data.submissionId);
      } else if (data.captured && data.submissionId) {
        trackGenerateLead(data.submissionId);
      }
    ...
    >
      ...
      {showSummary ? (
        <div
          ...
          tabIndex={-1}
          role="alert"
          className="mt-5 rounded-sm border border-error/30 bg-error-soft p-4 text-sm text-error outline-none"
        >
